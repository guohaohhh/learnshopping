...
``````
 封装返回前端的高复用对象 
     ...
     private String msg;
     //返回接口数据
     private T data;  因为不知道返回是个什么类型，定义为泛型（T）
 （1）get和set 方法   （2）无参和有参构造器   （3）（俩个对外的构造函数） 登录有两种状态（成功和失败）
 ``````
 # 注册
 ``````
 step1：参数的非空校验        接受数据：springMVC的数据绑定
 step2：效验用户名
 step3：效验邮箱
 step4：注册      给一个注册等级    密码进行加密
 step5：返回结果
 ``````
 # 登录
 ``````
 step1：参数的非空校验     //在apache提供的commons-lang包中定义了方法  StringUtils.isBlank()  和 StringUtils.isEmpty()
                          //StringUtils.isBlank()能判断空格为空    如果是ture，这个值就是空的
  step2：效验用户名    查询count（*） 大于0就有 <=0 就没有   按照用户名进行查找
  step3：根据用户名和密码查询   密码也进行加密对比
  step4：返回结果    //因为要查询出来，不能显示密码  所以要对密码进行制空
 ``````
 # 忘记密码
 ``````
 根据用户名获取密保问题——> 回答问题----> 修改密码
 一。获取密保问题    按照名字获取
 step1：参数的非空校验 
 step2：判断用户是否存在
 step3：查询密保问题     按照名字获取
 ...
 一。 按categoryId查询类别
 step1：非空校验
 step2：按categoryId查询类别    返回的是一个对象
 step3：查询子类别    返回的是一个list集合
 step4：返回结果
  ``````
 #  添加品类节点
  `````
